...
def water_and_weight(trials, subjects):
    ...
    df = pd.DataFrame(columns=['subject_uuid', 'weekend_water', 'weight_start',
                               'weight_loss'], index=range(len(mice)))  # , 'water_weight'
...
def performance_metrics(trials, session):
    ...
    for m, mouse in enumerate(mice):
        ...
        perf_init = mouse_data.loc[mouse_data['training_day'] == 1, 'performance_easy']
        if len(perf_init) > 0:
            d_matrix['perf_init'][m] = np.nanmean(perf_init)

        RT_init = list(mouse_data.loc[(mouse_data['training_day'] == 1) &
                                      (mouse_data['contrast'] >= 0.5), 'RT'])
        if len(RT_init) > 0:
            d_matrix['RT_init'][m] = np.nanmedian(RT_init)

        trials_init = mouse_data.loc[mouse_data['training_day'] == 1, 'trial_id']
        if len(trials_init) > 0:
            d_matrix['trials_init'][m] = np.max(trials_init)

        # Change in task performance across the first sessions
        perf_last = mouse_data.loc[mouse_data['training_day'] == session, 'performance_easy']
        ...
        RT_last = list(mouse_data.loc[(mouse_data['training_day'] ==
                                       session) & (mouse_data['contrast'] >= 0.5), 'RT'])
        ...
        trials_last = mouse_data.loc[mouse_data['training_day'] == session, 'trial_id']
